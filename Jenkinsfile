pipeline{
 environment {
 registry = "rrawl3s/vat-calculator-demo"
    ...
    agent any
        stages {
            stage('Checkout Code') {
                ...
                git branch: 'main', url: 'https://github.com/RRawles/lbg-pipeline-complete-1.git'
                }
            }
             ...
             stage ("Pull latest image"){
                steps {
                    script {
                        sshPublisher(publishers: [sshPublisherDesc(configName: 'targetDeploymentServer', 
                        transfers: [sshTransfer(cleanRemote: false, excludes: '', 
                        execCommand: 'docker pull rrawl3s/vat-calculator-demo:latest', 
                        execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, 
                        patternSeparator: '[, ]+', remoteDirectory: '', remoteDirectorySDF: false, removePrefix: '', 
                        sourceFiles: '')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, 
                        verbose: false)])
            ...
            stage ("Deploy to target GCP VM"){
                steps {
                    script {
                        sshPublisher(publishers: [sshPublisherDesc(configName: 'targetDeploymentServer', 
                        transfers: [sshTransfer(cleanRemote: false, excludes: '', 
                        execCommand: 'docker run --name deployed-vat-calc -d -p 8000:80 rrawl3s/vat-calculator-demo:latest', 
                        execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, 
                        patternSeparator: '[, ]+', remoteDirectory: '', remoteDirectorySDF: false, removePrefix: '', 
                        sourceFiles: '')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, 
                        verbose: false)])
